...
// ── Config ────────────────────────────────────────────────────────────────────
// In test mode (NODE_ENV=test) rate limits are disabled so integration tests
// can publish freely without triggering 429s. Never set NODE_ENV=test in prod.
...
export function publishRateLimit() {
  return (req, res, next) => {
    // Skip rate limiting in test mode so integration tests can run freely.
