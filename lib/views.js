...
async function withViewLock(slug, fn) {
  ...
  let release;
  _viewLocks.set(slug, new Promise((r) => { release = r; }));
  await prev;
  try {
    return await fn();
  } finally {
    release();
    // Clean up map entry after the lock chain drains
    if (_viewLocks.get(slug) === Promise.resolve()) {
 ...
 *
 * @param {string} slug
 * @param {string} ip           - visitor IP (used in fingerprint)
 * @param {string} userAgent    - visitor UA (used in fingerprint)
 ...
 */
export async function recordView(slug, ip, userAgent = '') {
  const date = today();
  const fp = fingerprintRequest(ip, userAgent, date);
